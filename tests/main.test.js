...
let settings = Object.assign(Object.assign({}, partialSettings), { randomization: Randomization.Bag, rotationSystems: { srs } });
let initialMetaState = Object.assign(Object.assign({}, partialMeta), { status: GameStatus.Ready });
let initialSettingsState = Object.assign(Object.assign({}, settings), { rotationSystems: Object.assign(Object.assign({}, settings.rotationSystems), { srs: Object.assign(Object.assign({}, settings.rotationSystems.srs), { previewGrids: expectedPreviewGridSettings }) }) });
let initialActivePiece = {
    id: null,
    location: null,
    coordinates: [],
    ghostCoordinates: [],
...
let emptyGridIsValid = (grid, rows, columns) => {
...
describe('Testing initialization', () => {
    test('State should match expected initial properties', () => {
         ...
         * Init
         */
        let initializedState = execute(null, ...LifecycleProviders.Makers.init(settings));
        ...
        let preparedState = execute(initializedState, PreviewDrafters.Makers.RandomNumbers.add(...randomNumbers), ...PreviewProviders.Prepare.prepareQueue);
        expect(preparedState.preview).toEqual(expectedPreparedPreviewState);
        /**
         * Sequenced steps
         */
        let nextState = execute(preparedState, ...CompositeProviders.Next.next);
        // let dropState: State = execute(nextState, ...PlayfieldProviders.drop);
        // console.log(dropState.playfield.grid)
        let shiftState = execute(nextState, LifecycleProviders.Makers.startInput(Input.ShiftLeft), ...LifecycleProviders.Makers.endInput(Input.ShiftLeft));
        console.log("next");
        let shiftState2 = execute(shiftState, LifecycleProviders.Makers.startInput(Input.ShiftLeft), ...LifecycleProviders.Makers.endInput(Input.ShiftLeft));
        console.log("next2");
        let shiftState3 = execute(shiftState2, LifecycleProviders.Makers.startInput(Input.ShiftLeft), ...LifecycleProviders.Makers.endInput(Input.ShiftLeft));
