...
export default (pieces: { [id: PieceIdentifier]: PieceSpec }) => {
    return (params: { pieceId: PieceIdentifier, state: CoreState & GeneratedGrids }) => {
        const { spawnOrientation, startLocation } = pieces[params.pieceId]
        return {
            grid: params.state.generatedGrids[params.pieceId][spawnOrientation],
            orientation: spawnOrientation,
            location: startLocation
        } satisfies PieceSpawnInfo
    }
}