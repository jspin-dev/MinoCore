...
export const rootOperation = (pieceId: PieceIdentifier) => {
    return mapOperation((result: CoreResult, { schema, operations }: CoreDependencies) => {
        ...
        const pieceSpec = schema.rotationSystem.pieces[pieceId]
        const { spawnOrientation, startLocation } = pieceSpec
        const coordinates = gridToList(pieceSpec.grids[spawnOrientation], startLocation.x, startLocation.y, 1)
        ...
        return sequence(
            setNewActivePiece(pieceId, spawnOrientation, startLocation, coordinates),
...
const setNewActivePiece = (
    ...
    const distanceCalculationInfo = { coordinates, playfield: state.playfield, playfieldDimens: schema.playfieldDimens }
    ...
    const playfield = mapPlayfield({
        ...
        map: (cell, coordinate) => Coordinate.includes(coordinates, coordinate) ? Cell.Active(pieceId) : cell
    })
    return {
        state: {
            ...state,
